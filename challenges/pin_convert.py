...
def convert_word(word):
    ...
    final_lst = [] # Creates a new empty list 
    for item in range(len(con_pin)):
        """In order to create a two-digit number for numbers <10, we need
        to insert a '0' in front of the digit."""
        if fin_pin[item] < 10: 
            fin_pin[item] = '0' + str(fin_pin[item]) 
        else:
            final_lst.append(fin_pin[item])
